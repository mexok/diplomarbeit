(function() {
    ...
    var loc_dialogCloseWindow = "Close this dialog";
    ...
    var loc_signInToSeeProfile = "<a class='docs-sign-in' href=\"#\">Sign in</a> to view your user profile.";
    ...
    var buttonTemplate = '\n\t<button type="button" class="dialog-close has-default-focus has-inverted-focus" aria-label="' + loc_dialogCloseWindow + '">\n\t\t<span class="docons docons-navigate-close"></span>\n\t</button>\n';
    var Dialog = function() {
        function Dialog(bus, container, options) {
            if (bus === void 0) {
                bus = eventBus;
            }
            if (options === void 0) {
            ...
            this.bus = bus;
            this.active = false;
            if (options.existingDOMElement && document$1.body.contains(options.existingDOMElement)) {
                this.existingDOMElement = options.existingDOMElement;
                this.existingDOMElement.setAttribute("tabindex", "-1");
                return;
            }
            ...
            this.overlay.classList.add("has-default-focus");
            this.overlay.classList.add("modal-overlay");
            this.overlay.classList.add("has-inverted-focus");
        }
        Dialog.prototype.show = function(view, options) {
            ...
            if (this.existingDOMElement) {
                this.container = this.wrapper = this.existingDOMElement;
            } else {
                ...
                if (options.mobileFull) {
                    this.container.classList.add("fill-mobile");
                }
            }
            this.wrapper.addEventListener("click", function(event) {
                var target = event.target;
                if (target.closest(".dialog-close") || target.closest(".modal-overlay") || target.closest(".modal-close") || target.closest(".modal-close-control") || target.closest(".modal-background")) {
                    event.preventDefault();
                    _this.hide();
                }
            });
            ...
            if (this.existingDOMElement) {
                this.wrapper.parentElement.insertBefore(createTabSentinel(document$1), this.wrapper);
                this.wrapper.parentElement.insertBefore(createTabSentinel(document$1), this.wrapper.nextElementSibling);
            ...
            this.bus.publish(new DialogOpenEvent());
        };
        Dialog.prototype.hide = function() {
            ...
            this.active = !this.active;
            this.bus.publish(new DialogCloseEvent());
        };
        return Dialog;
    }();
    var DialogOpenEvent = function() {
    ...
    function handleModals() {
        window.addEventListener("click", function(e) {
            var target = e.target;
            if (target.classList.contains("modal-close") || target.classList.contains("modal-close-control") || target.classList.contains("delete") || target.classList.contains("modal-background")) {
                removeModal(target);
            }
        });
    ...
    function removeModal(target) {
        ...
        if (modal) {
            ...
            Array.from(document.body.querySelectorAll(".tab-sentinel")).forEach(function(element) {
    ...
    function activateModal(modal) {
        focusHandler = constrainFocus(modal);
        modal.parentElement.insertBefore(createTabSentinel(document), modal);
        modal.parentElement.insertBefore(createTabSentinel(document), modal.nextElementSibling);
    ...
    function initializeLightBox(mainElement) {
        ...
        var bus = new EventBus();
        bus.subscribe(DialogCloseEvent, browser.close.bind(browser));
    ...
    var PagingControl = function() {
        function PagingControl(eventBus, element, currentPage, lastPage) {
            var _this = this;
            this.currentPage = 1;
            this.lastPage = 1;
            ...
            element.addEventListener("click", function(event) {
                ...
                _this.eventBus.publish(pageEvent);
            });
            this.create();
        }
    ...
    function populateBreadcrumb(breadcrumbHolder, title, module) {
        ...
        var singleLearningPath = module && module.parents.length === 1 ? template$2(module.parents[0].title, "/" + userLocale + module.parents[0].url) : "";
        switch (pageKind) {
          case "product":
            breadcrumbHolder.innerHTML = template$2("Docs", "/" + userLocale + "/") + template$2("Learn");
            break;

          case "browse":
            breadcrumbHolder.innerHTML = template$2("Docs", "/" + userLocale + "/") + template$2("Learn", "/" + userLocale + "/learn/") + template$2(loc_browse);
            break;

          case "path":
            breadcrumbHolder.innerHTML = template$2("Docs", "/" + userLocale + "/") + template$2("Learn", "/" + userLocale + "/learn/") + template$2(loc_browse, "/" + userLocale + "/learn/browse/") + template$2(title);
            break;

          case "module":
            breadcrumbHolder.innerHTML = template$2("Docs", "/" + userLocale + "/") + template$2("Learn", "/" + userLocale + "/learn/") + template$2(loc_browse, "/" + userLocale + "/learn/browse/") + singleLearningPath + template$2(title);
            break;

          case "unit":
            breadcrumbHolder.innerHTML = template$2("Docs", "/" + userLocale + "/") + template$2("Learn", "/" + userLocale + "/learn/") + template$2(loc_browse, "/" + userLocale + "/learn/browse/") + singleLearningPath + template$2(module.title, "/" + userLocale + module.url) + template$2(title);
            break;
    ...
    function template$2(title, link) {
        var crumb;
        if (link) {
    ...
    function debounceScheduler(scheduler, clearScheduler, handler, timeout) {
        ...
        var caller = function() {
            ...
            scheduleId = scheduler.apply(null, [ function() {
    ...
    var FacetList = function() {
        function FacetList(facetContainer, tagContainer, bus, filterCriteria) {
            ...
            this.bus = bus;
            this.bus.subscribe(FilterUpdatedEvent, function(event) {
                _this.filterCriteria = event.filterCriteria;
                _this.facetContainer.innerHTML = facetListTemplate(event.facets, event.filterCriteria);
                addFacetTag(_this.tagContainer, getTermsFromFilters$1(event.filterCriteria));
                Array.from(_this.facetContainer.querySelectorAll("button[aria-controls]")).forEach(function(element) {
                    expander(element);
                });
            });
            facetContainer.addEventListener("click", function(event) {
                if (event.target instanceof HTMLInputElement && event.target.type === "checkbox") {
                    ...
                    bus.publish(new FacetSelectedEvent(event.target.name, list.filter(function(x) {
    ...
    var FilterUpdatedEvent = function() {
        function FilterUpdatedEvent(facets, filterCriteria) {
            this.facets = facets;
            this.filterCriteria = filterCriteria;
        }
    ...
    function contentBrowserPage(containerElement) {
        ...
        eventBus.subscribe(FacetSelectedEvent, function(event) {
            var _a;
            filterCriteria[event.facetName] = event.filters;
            updateQueryString((_a = {}, _a[event.facetName] = event.filters.join(","), _a.page = null, 
            _a), "pushState");
        });
        eventBus.subscribe(FacetSelectedEvent, debounce.timeout(function(event) {
            var query = parseQueryString();
            var terms = query.term && query.term.length ? query.term.trim() : null;
            eventBus.publish(new SearchRequestEvent({
                terms: terms,
        ...
        contentLoaded.then(function() {
            ...
            var facetListComponent = new FacetList(facetList, form, eventBus, filterCriteria);
    ...
    function handleSearchResponseEvent(_a) {
        ...
        contentLoaded.then(function() {
            ...
            var pagingElement = container.querySelector("#results-paging");
            var refineElement = container.querySelector("#refine-content");
            eventBus.publish(new FilterUpdatedEvent(result.count ? result.facets : convertFilterCriteriaToFalseFacets(filterCriteria), filterCriteria));
        ...
        Quiz.prototype.handleValidationEvent = function(event) {
            ...
            if (event.passed) {
                ...
                if (event.achievements && event.achievements.length) {
                    ...
                    xpTag.innerHTML = loc_xp.replace("{0}", xp.toString());
                }
                activateModal(modal);
            ...
            return __generator(this, function(_f) {
                switch (_f.label) {
                  ...
                  case 1:
                    ...
                    expanderToggles = initExpanders(container, 0);
    ...
    function initExpanders(container, openIndex) {
    ...
    function modulePage() {
        ...
        Promise.all([ loadModuleInformation(moduleId), contentLoaded ]).then(function(_a) {
            ...
            if (!user.isAuthenticated) {
                renderStartContinueUnitButton(document.querySelector(".abstract"), "/" + userLocale + module.units[0].url, loc_start, "start");
            }
            user.whenAuthenticated().then(function() {
                if (module.status === "notStarted") {
                    ...
                    if (module.status === "inProgress") {
                        for (var _i = 0, _a = module.units; _i < _a.length; _i++) {
                            var unit = _a[_i];
                            if (unit.status !== "completed") {
                                renderStartContinueUnitButton(document.querySelector(".abstract"), "/" + userLocale + unit.url, loc_continue, "continue");
                                break;
                            }
                        }
                    } else {
                  ...
                  case 2:
                    ...
                    if (!sandbox.resourceGroupId) {
                        if (result.requestVerb === "GET") {
                            ...
                            if (document$1.referrer.indexOf("https://invitations.microsoft.com/redeem/redeeming") === 0) {
                                jsllReady.then(function(awa) {
                                    return awa.ct.captureContentPageAction({
                                        content: {
    ...
    function completeButtonLogic(module, buttonHolder, buildModal) {
        ...
        if (module.parents.length === 1) {
            ...
            if (mobileQuery$1.matches || module.parents.length === 0) {
                ...
                buttonHolder.innerHTML = "<p>" + loc_exploreOtherModules + "</p>";
    ...
    function taskValidationModal(complete, currentUnit, nextUnitUrl, errorHtml) {
        ...
        if (complete) {
            ...
            taskButtonText.textContent = loc_continue;
    ...
    function moduleUnitPage() {
        ...
        Promise.all([ currentModule, contentLoaded ]).then(function(_a) {
            ...
            bus.subscribe(ProgressUpdatedEvent, function(data) {
                ...
                buildNextCTA(module, unitIndex, completionType, interactiveType ? interactiveType.name : null, numberCompleted, isReturnModuleVisit);
            });
    ...
    function signInTemplate() {
        return "<p>" + loc_signInToSeeProfile + "</p>";
    }
    function authenticatedTemplate() {
        return '\n\t\t<h1 id="my-profile-name"></h1>\n\t\t<p id="my-profile-id"></p>\n\t';
    }
    function progressBarTemplate() {
        return '\n\t<div class="c-progress f-indeterminate-regional" role="progressbar" aria-valuetext="' + loc_loading + '" tabindex="0" aria-label="indeterminate regional progress bar">\n\t\t<span></span>\n\t\t<span></span>\n\t\t<span></span>\n\t\t<span></span>\n\t\t<span></span>\n\t</div>\n\t';
    }
