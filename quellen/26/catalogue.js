$(document).ready(function() {
  ...
  $(document).on('click', '#redirectionUrl', function() {

    // GTM: Push dataLayer
    if (document.getElementById('csNumber')) {
      var eventAction = document.getElementById('csNumber').innerHTML;
    }
    if (document.getElementById('pubNumber')) {
      var eventAction = document.getElementById('pubNumber').innerHTML;
    }

    var eventLabel = $(this).attr('data-storeloc');
    var eventValue = $('#productPrice').text();

    window.dataLayer.push({
      'event': 'eventGA',
      'eventCategory': 'webstore-oo',
...
function addToBasket(counter, guilang, prodType) {
  ...
  .done(
    function(output, status, xhr) {
      ...
      $("#redirectionUrl").attr("data-storeloc", output.country_iso);
    }
  ).fail(
    function(xhr, status, errorThrown) {
